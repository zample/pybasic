...
- 使用类访问绑定类的方法时，如果类方法中需要访问当前类的成员，可以通过__class__成员名来
...
- 私有
    ...
    - 可以使用对象名._classname_attributename访问        
- 受保护的封装，protected
...
- 继承的特征
    ...
    - 子类如果想扩充父类的的方法，可以在定义新方法的同时访问父类成员来进行代码重用，可以使用 [父类名.父类成员] 的格式来调用父类成员，也可以使用super().父类成员的格式来调用                      
- 继承变量函数的查找顺序问题
    - 优先查找自己的变量
    - 没有则查找父亲
...
- 菱形继承和钻石继承问题
    - 多个子类继承自同一个父类，这些子类同一个类继承，于是继承关系图形形成一个菱形图谱
    - 关于多继承的MRO
        - 关于多继承的MRO
...
- 无论哪种修饰符都是为了对成员属性进行相应的控制
    ...
    - property：使用单前类中使用，可以控制一个类中多个属性
...
- __new__:对象实例化方法
- __call__：对象当函数使用时触发                                                                 
- `__str__`:当对象被当成字符串时调用
- `__repr__`:返回字符串，和`__str__`具体区别请百度
- 描述符相关
    - `__set__`
    - `__get__`
    - `__delete__`
